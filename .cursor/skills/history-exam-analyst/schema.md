...
- CSV 매핑: `성취기준_코드`만 분류표에서 사용 (원문은 CSV에 넣지 않음). `evaluation_element`·`answer`는 참고용
- `문제형식`, `세부형식`, `시대`, `자료핵심요소`, `정답핵심요소`는 **`exam_analysis.json`만** (AI, 크롭 PNG 기반). 샤드는 `merge_exam_analysis_shards.py`로 병합
